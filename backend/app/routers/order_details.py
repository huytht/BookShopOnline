...
async def list_order_details(request: Request):
    ...
    for order_detail in await request.app.mongodb["order_details"].find().to_list(length=100):
        if isinstance(order_detail["_id"], int):
            if (book := await request.app.mongodb["book"].find_one({"_id": order_detail['book_id']})) is not None:
...
async def update_order_detail(id: int, request: Request, order_detail: OrderDetailsUpdateModel = Body(...)):
    order_detail = {k: v for k, v in order_detail.dict().items() if v is not None}
...
async def delete_order_detail(id: int, request: Request):
    ...
    raise HTTPException(status_code=404, detail=f"order_details {id} not found")
